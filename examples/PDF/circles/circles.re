...
let points = (x, y, r, step) => {
  let n = ref(0)
  and points = ref([]);
  while (float_of_int(n^) *. step < 2. *. r *. pi) {
...
let circle = (x, y, r) =>
  switch (points(x, y, r, pi /. 20.)) {
  | [(x, y), ...lines] =>
    [
      [@implicit_arity] Pdfpage.Move(x, y),
      ...List.map(((x, y)) => [@implicit_arity] Pdfpage.Line(x, y), lines),
    ]
    @ [Pdfpage.Close]
  | _ => assert(false)
  };

let circle_filled = circle(300., 300., 100.) @ [Pdfpage.Fill];

let _ = Pdfwrite.pdf_to_file(basic_document(circle_filled), "q1.pdf");

/* Q 2. Page covered in random circles. */
let random_circle = () => {
  let x = Random.float(700.) -. 50.
  and y = Random.float(1000.) -. 50.
  and r = Random.float(100.) +. 20.;
  circle(x, y, r);
};
...
let _ = Pdfwrite.pdf_to_file(basic_document(many_circles), "q2.pdf");
...
let _ = Pdfwrite.pdf_to_file(basic_document(many_circles_colour), "q3.pdf");
...
let circle_over_circles = many_circles_colour @ big_circle;

let _ = Pdfwrite.pdf_to_file(basic_document(circle_over_circles), "q4.pdf");
...
let clipped = big_clipping_circle @ many_circles_colour;

let _ = Pdfwrite.pdf_to_file(basic_document(clipped), "q5.pdf");
