let box = [
  [@implicit_arity] Pdfpage.Move(100., 100.),
  [@implicit_arity] Pdfpage.Line(100., 200.),
  [@implicit_arity] Pdfpage.Line(200., 200.),
  [@implicit_arity] Pdfpage.Line(200., 100.),
  Pdfpage.Close,
  Pdfpage.Fill,
];

let hello_text = "1 0 0 1 50 770 cm BT /F0 36 Tf (Hello, World!) Tj ET";

let hello_text2 = "1 0 0 1 50 770 cm BT /F0 36 Tf (Hello, World 2!) Tj ET";

let hello_text3 = "1 0 0 1 50 770 cm BT /F0 36 Tf (Hello, World 3!) Tj ET";
...
let objects3 = (str, str2, str3) => [
  ...
  (
    ...
    Pdf.Dictionary([
      ...
      (
        "/Kids",
        Pdf.Array([Pdf.Indirect(1), Pdf.Indirect(6), Pdf.Indirect(7)]),
      ),
      ("/Count", Pdf.Integer(3)),
    ]),
  ),
  ...
  (
    ...
    [@implicit_arity]
    Pdf.Stream(
      Pdf.Dictionary([("/Length", Pdf.Integer(String.length(str2)))]),
      str2,
    ),
  ),
  (
    ...
    [@implicit_arity]
    Pdf.Stream(
      Pdf.Dictionary([("/Length", Pdf.Integer(String.length(str3)))]),
      str3,
    ),
  ),
];
...
let mkline = (x, y, x1, y1) => [
  [@implicit_arity] Pdfpage.Move(x, y),
  [@implicit_arity] Pdfpage.Line(x1, y1),
  Pdfpage.Stroke,
];
...
let graph_string = (w, h, step) => {
  let horizontals = map(n => mkline(0., n, w, n), steps(h, step))
  and verticals = map(n => mkline(n, 0., n, h), steps(w, step));
...
let rec rotate_90 = obj =>
  ...
  | [@implicit_arity] Pdf.Stream(dict, str) =>
    [@implicit_arity] Pdf.Stream(rotate_90(dict), str)
  | x => x
  }
