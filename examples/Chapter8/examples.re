...
let string_of_point = p => Printf.sprintf("%s = (%f, %f)", p.label, p.x, p.y);
...
let print_line = (a, b, c) => Printf.printf("%6i| %6i| %6i\n", a, b, c);

let print_nums = nums => {
  print_header();
  List.iter(((a, b, c)) => print_line(a, b, c), nums);
};
...
let print_point = p => Printf.printf("%s = (%f, %f)%!", p.label, p.x, p.y);
