...
let rec equal = (l, len, p) =>
  len == 0
  || List.hd(l) == List.hd(p)
  && equal(List.tl(l), len - 1, List.tl(p));

let rec search_list_inner = (p, len_p, l, len_l) =>
  len_p <= len_l
  && (
    equal(l, len_p, p) || search_list_inner(p, len_p, List.tl(l), len_l - 1)
  );
...
let swallow_all = (ch, s, sp) => {
  let x = ref(sp);
  while (x^ < String.length(s) && s.[x^] == ch) {
    x := x^ + 1;
  };
  x^ - sp;
};
...
let rec at = (p, pp, s, sp) =>
  pp > String.length(p)
  - 1  /* used whole pattern */
  || (
    switch (
      switch (p.[pp]) {
      | '?' =>
        if (pp + 1 > String.length(p) - 1) {
          None;
        } else if
          /* ? at end of pattern */
          (sp > String.length(s) - 1) {
          Some((2, 0));
        } else if
          /* No char left to match */
          (p.[pp + 1] == s.[sp]) {
          Some((2, 1));
        } else {
          /* match */
      ...
      | '*' =>
        if (pp + 1 > String.length(p) - 1) {
          ...
          /* * at end of pattern */
          Some((2, swallow_all(p.[pp + 1], s, sp)));
...
let test = () =>
  List.iter(
    ((pattern, str, expected)) => {
      let result = search(pattern, str);
      Printf.printf(
        "Pattern %-10sString %-10s Expected %-14b   Actual %-14b   Correct %-14b\n",
        pattern,
        str,
        expected,
        result,
